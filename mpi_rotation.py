...
def plot_comparison(original, rotated, title=""):
    ...
    ax1.scatter(
        original[:, 0], original[:, 1], original[:, 2], c="blue", s=20, alpha=0.6
    )
...
def main():
    ...
    R = (
        R_arbitrary([1, 1, 0], args.angle)
        if args.axis == "arbitrary"
        else R_map[args.axis](args.angle)
    )
    ...
    if rank == 0:
        if args.object == "cube":
            points = make_cube(args.n or 500)
    ...
    if rank == 0:
        ...
        if args.plot:
            plot_comparison(
                original_copy, final_result, f"{args.axis.upper()} {args.angle}°"
            )
